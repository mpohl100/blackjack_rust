...
async fn play_blackjack_through_service_class() {
    ...
    let mut game_info = blackjack_service
        .play_game(create_game_response.game_id, "".to_owned())
        .await;

    for _ in 0..10 {
        // play 10 actions
        let action = "h".to_owned();
        game_info = blackjack_service
            .play_game(create_game_response.game_id.clone(), action)
            .await;
    ...
    // delete the game
    let delete_response = blackjack_service
        .delete_game(create_game_response.game_id)
        .await;
